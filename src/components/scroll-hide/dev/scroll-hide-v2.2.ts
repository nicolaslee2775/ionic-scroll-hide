...
class Item {
	...
	public shrinkStart = 0;
	public translateStart = 0;
	public moveStart = 0;
	...
	constructor(public ele: HTMLElement) {
		...
		if(shrinkable) {
			this.transition = ItemTransition.Shrink;
	...
	setY(shrinkStart: number, translateStart: number) {
		this.shrinkStart = shrinkStart;
		this.translateStart = translateStart;
	}

	setShrinkStart(shrinkStart: number) {
		this.shrinkStart = shrinkStart;
	}
	setTranslateStart(translateStart: number) {
		this.translateStart = translateStart;
	}
	setMoveStart(moveStart: number) {
...
export class ScrollHide implements OnInit, OnDestroy {
	...
	private init() {
		...
		this.headerItems.forEach((item, index) => {
			...
			case ItemTransition.Shrink:
				let shrinkHeight = (item.height * item.scrollShrinkVal);
				item.setShrinkStart(shrinkableHeightSum);
				item.setTranslateStart(shrinkableHeightSum + shrinkHeight);							
				item.setMoveStart(shrinkableHeightSum + shrinkHeight);							
				shrinkableHeightSum += shrinkHeight;
				break;

			case ItemTransition.Translate:
				item.setShrinkStart(shrinkableHeightSum);
				item.setTranslateStart(shrinkableHeightSum);
				item.setMoveStart(shrinkableHeightSum + item.height);											
				shrinkableHeightSum += item.height;				
				break;

			case ItemTransition.Static:
				item.setShrinkStart(shrinkableHeightSum);
				item.setTranslateStart(shrinkableHeightSum);
				item.setMoveStart(shrinkableHeightSum);															
				break;
			}
	...
	private resetStyle() {
		...
		this.headerItems.forEach((item) => {
			...
			if(item.transition === ItemTransition.Translate) {	
	...
	private modifyDom(y: number, duration: number, dowWrite: DomWrite) {
		dowWrite(() => {
			...
			this.headerItems.forEach((item) => {
				...
				//let dy = constrain((y - item.shrinkStart), 0, Infinity);
				//let val = constrain((dy / item.height), 0, 1);										
				
				if(item.type === ItemType.Tabbar) {
					let dy_shrink 	 = constrain((y - item.shrinkStart), 	 0, (item.translateStart - item.shrinkStart));
					let dy_translate = constrain((y - item.translateStart), 0, (item.moveStart - item.translateStart));
					let dy_move 	 = constrain((y - item.moveStart), 	     0, Infinity);					
					...
					if(y <= item.translateStart) {
					...
					} else if(y <= item.moveStart){ 
						// Translating
						
						let val = constrain((dy_translate / item.height), 0, 1);																
						$(item.ele).css({
							height: item.height - dy_translate,
						...
						$(item.ele).css({
							height: item.height - Math.max((item.translateStart - item.shrinkStart), (item.moveStart - item.translateStart)),
							//minHeight: item.height - (item.translateStart - item.shrinkStart),
							//marginBottom: (item.translateStart - item.shrinkStart),
						})
					...
					/*if(y <= item.translateStart) { 
						...
						$(item.ele).css({
							height: 	  item.height - (item.translateStart - item.shrinkStart),
							minHeight: 	  item.height - (item.translateStart - item.shrinkStart),
							marginBottom: (item.translateStart - item.shrinkStart),
						})
					...
					if(y <= item.translateStart) { 
						...
						let maxPrevVal = Math.max((item.translateStart - item.shrinkStart), (item.moveStart - item.translateStart));
						$(item.ele).css({
							height: 	  item.height - maxPrevVal,
							minHeight: 	  item.height - maxPrevVal,
							marginBottom: maxPrevVal,
						})
						this.translateElementY(item.ele, -dy_move, duration);
					}
				}
