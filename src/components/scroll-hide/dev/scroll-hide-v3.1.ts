...
export class ScrollHide implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		/*(<any>this.content._tabs).ionChange.subscribe(() => {
			console.log("ionChange > viewEntered:", this.viewEntered);
			if(this.viewEntered && this.cachedTabbarStayle) {
				//this.appplyStoredTabbarStyle();	
				if(this.content._tabsPlacement === "top") {
					let top = parseInt(this.cachedTabbarStayle.top);
					console.log("this.content._tabsPlacement === 'top' top:", top);					
					this.content._tabs.setTabbarPosition(top, -1);
				}
			}
		});*/
		...
		this.subscriptions.push(
			this.viewCtrl.willLeave.subscribe(() => {
				...
				//this.storeTabbarStyle();
				//this.resetTabbarStyle();
			})
		);
		...
		// Prevent multipe trigger of scrollEnd event

		//var startTimestamp: number = 0;
		var prevEndTimestep: number = 0;
		this.subscriptions.push(
			this.content.ionScrollStart.subscribe(event => {
				//console.log("ionScrollStart!", event);
				//startTimestamp = event.timeStamp;
				//console.log("ionScrollStart!", event.timeStamp);			
			})
		);
		this.subscriptions.push(
			this.content.ionScroll.subscribe(event => {
				if(event.timeStamp > prevEndTimestep + 200) { // prevent multiple triggered of scroll event
					...
					//console.log("ionScroll!", event.timeStamp);
				} else {
					//console.log("fake --> ionScroll!", event.timeStamp);
				}		
				//this.onContentScroll(event, false);
			})
		);
		this.subscriptions.push(
			this.content.ionScrollEnd.subscribe(event => {
				if(event.timeStamp > prevEndTimestep + 200) {
					...
					//console.log("ionScrollEnd!", event.timeStamp);
				} else {
					//console.log("fake --> ionScrollEnd!", event.timeStamp);
				}
		...
		win.temp.viewCtrl = this.viewCtrl;
	}
	...
	onContentScroll(event: ScrollEvent, isMoveEnd: boolean) {
		...
		//console.log(event.scrollTop, "maxScrollTop:", maxScrollTop, "this.hasScrollToBottomBefore:", this.hasScrollToBottomBefore);
		//console.log(event.scrollTop + ", maxScrollTop: " + maxScrollTop + ", hasExtend: " + this.extendBottom + ", hasBottom: " + this.hasScrollToBottomBefore);
		...
		/*if (event.scrollTop < maxScrollTop - 40) {
		...
		} else if (event.scrollTop >= maxScrollTop) {
			...
			if(isMoveEnd) {
				...
				console.log("--> extend padding-bottom");
			}
		}*/
		...
		/*if(event.scrollTop <= maxScrollTop) {
			if(this.hasBeyondMaxScrollTop) {
				console.log("--> 2 extend padding-bottom!");				
				this.extendBottom = true;
			}
			...
			this.hasBeyondMaxScrollTop = true;
		...
		// ????
		if(event.scrollTop >= maxScrollTop) {
			if(event.scrollTop < this.scrollTopPrev) {
				console.log("--> scroll to bottom!");
				this.hasScrollToBottomBefore = true;			
				this.content.setScrollElementStyle("padding-bottom", (this.content._pBottom + this.footerHeight) + "px");

			} else if(this.hasScrollToBottomBefore) {
				console.log("--> show!");				
				y = 0;
				duration = this.defaultDuration;
			} 
		} else if(event.scrollTop < maxScrollTop - 40){
			console.log("--> show!");
		...
		if(event.scrollTop >= maxScrollTop) {
			...
			if(this.hasScrollToBottomBefore) {
				this.hasScrollToBottomBefore = false;
				console.log("--> extend padding-bottom!");				
				this.hasExtendBottom = true;
				this.content.setScrollElementStyle("padding-bottom", (this.content._pBottom + this.footerHeight) + "px");
			} else {
				console.log("reset padding-bottom");
				this.hasExtendBottom = false;
				this.content.setScrollElementStyle("padding-bottom", (this.content._pBottom) + "px");
			}
		}*/
		...
		if(event.scrollTop >= maxScrollTop) {
			if(scrollTopDiff <= 0) {
				console.log("--> Beyond MaxScrollTop! extend!");
				this.hasBeyondMaxScrollTop = true;
				this.extendBottom = true;
			} else { // > 0
				if(this.hasBeyondMaxScrollTop) {
			...
			if(scrollTopDiff <= 0) {
				console.log("--> reset beyondMaxScrollTop!");				
				this.hasBeyondMaxScrollTop = false;				
			}
		}
		...
		/*if(this.hasStopMoving) {
			if(!this.hasClearStopMoving) {
				console.log("---> clear stop moving!");			
				this.hasClearStopMoving = true;
				this.content.setScrollElementStyle("overflow-y", "");			
			}

			if(scrollTopDiff < 0) { // scroll up
				this.hasStopMoving = false;
			
			} else if(scrollTopDiff > 0) { // scroll down
				this.hasStopMoving = false;
				console.log("--> show!");
				y = 0;
				duration = this.defaultDuration;
