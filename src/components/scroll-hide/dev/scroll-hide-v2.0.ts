	...
	private init() {
		...
		this.headerItems.forEach((item, index) => {
			...
			if(item.shrinkable) {
				...
				item.setEndY(Infinity); //item.setEndY(shrinkableHeightSum + item.ele.offsetHeight);			
				shrinkableHeightSum += item.ele.offsetHeight;
			} else {
				item.setStartY(shrinkableHeightSum);
				item.setEndY(Infinity); //item.setEndY(shrinkableHeightSum);
			}
